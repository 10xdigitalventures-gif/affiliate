-- Supabase RLS hardening. Table owners (including Prisma's migration role)
-- bypass RLS because FORCE ROW LEVEL SECURITY is intentionally not enabled.
...
-- Supabase creates these Data API roles. Plain PostgreSQL installations do
-- not, so apply role-specific revocations only when each role exists. Do not
-- create login-facing Supabase roles in non-Supabase environments.
DO $migration$
